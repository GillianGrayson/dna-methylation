...
for marker_id, marker in tqdm(enumerate(markers)):
    ...
    search = GridSearchCV(estimator=model_type, scoring=scoring, param_grid=grid, cv=cv, verbose=3, n_jobs=4)
    ...
    Path(f'{path}/all_data/DNAmGrimAgeLike/markers/{target_part}/{marker}').mkdir(parents=True, exist_ok=True)
    model_df.to_excel(f'{path}/all_data/DNAmGrimAgeLike/markers/{target_part}/{marker}/clock.xlsx', index=False)
    ...
    df_pheno.to_excel(f'{path}/all_data/DNAmGrimAgeLike/markers/{target_part}/table.xlsx', index=True)

    metrics_df = pd.DataFrame(metrics)
    metrics_df.to_excel(f'{path}/all_data/DNAmGrimAgeLike/markers/{target_part}/params.xlsx', index=False)
